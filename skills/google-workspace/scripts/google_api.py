#!/usr/bin/env python3
"""Provider-neutral Google Workspace API CLI.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing JSON contract and falls back to the Python client
...
from _storage import get_google_workspace_home
...
def _gws_binary() -> str | None:
    override = os.getenv("GOOGLE_WORKSPACE_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API")
