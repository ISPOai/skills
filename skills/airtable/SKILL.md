...
4. Configure the token as an `AIRTABLE_API_KEY` secret or environment
   variable in the current project/agent host. Do not write it into source
   control, command history, or chat output:
   ```
...
`-s` suppresses curl's progress bar — keep it set for every call so the agent
output stays clean. Pipe through `python3 -m json.tool` (when Python is
available) or `jq` (if installed) for readable JSON.
...
- **Use an available shell-command tool with `curl`.** Generic web-fetch and
  browser tools may not support private authentication headers.
- **Verify `AIRTABLE_API_KEY` is available to the command subprocess** before
  making a request. If it is absent, ask the user to configure it through the
  project's secret/environment controls; never ask them to paste the token
  into chat or commit it to a file.
