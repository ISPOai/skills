...
- `references/banned-words.json`: the words and phrases to avoid. Check copy against this list before handing it back. It is a copy-quality list, not a spam filter workaround: what it flags is writing to fix, not mail that will be junked. Deliverability questions go to the `email-deliverability` skill in this catalog.

## Layers on top of

The companion `writing-quality` skill in this catalog carries the surface-independent rules: AI tells, plain-English swaps, front-loading, concrete over abstract, voice. Load it too; this skill only adds what is specific to email.
