...
- The user must fill `~/.xurl` with secrets manually on their own machine. In
  containers or remote runtimes, this must be the home directory seen by the
  agent's command-execution process.
...
`xurl` is a declared prerequisite. If it is absent, report the requirement
instead of running an installer automatically. When the user asks for setup
help, direct them to the official installation options and let them choose and
run one outside the agent session.
...
> **Container/remote HOME pitfall:** `~/.xurl` must be created under the same
> home directory that the agent's command-execution process uses. Have the user
> run setup in that environment, then verify only with `xurl auth status`; never
> inspect or copy the credential file into model context.
...
- **Token storage:** `~/.xurl` is YAML. In containers, ensure setup and agent
  commands use the same home directory. Never read or send this file to model
  context.
...
- Catalog adaptation: provider-specific runtime paths were replaced while the
  upstream safety guardrails were preserved.
