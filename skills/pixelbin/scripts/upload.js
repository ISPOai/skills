 ...
 *   1. URL mode  — reads project-owned image-urls.json (or video-urls.json) and uploads each via urlUpload.
 ...
 *
 * Output: project-owned JSON  →  { key: permanent_cdn_url }
 */
const fs = require('fs');
...
if ((filesArg === -1 || !argv[filesArg + 1]) && (sourceArg === -1 || !argv[sourceArg + 1])) {
    console.error('Pass --files <project-glob> or --source <project-owned-urls.json>.');
    process.exit(1);
}

const TOKEN = process.env.PIXELBIN_API_TOKEN;
const CLOUD = process.env.PIXELBIN_CLOUD_NAME;
const FOLDER = process.env.PIXELBIN_FOLDER || 'agent-skill';
const OVERWRITE = process.argv.includes('--overwrite');

if (!TOKEN || !CLOUD) {
    console.error('✗ Missing PIXELBIN_API_TOKEN or PIXELBIN_CLOUD_NAME in the host environment or project .env');
    process.exit(1);
}
...
async function main() {
    ...
    if (filesArg !== -1 && argv[filesArg + 1]) {
        ...
        const out = path.resolve(outArg !== -1 && argv[outArg + 1] ? argv[outArg + 1] : 'cdn-image-urls.json');
    ...
    // URL mode (default)
    const sourceFile = argv[sourceArg + 1];
    ...
    const outArg = argv.indexOf('--out');
    const out = path.resolve(outArg !== -1 && argv[outArg + 1] ? argv[outArg + 1] : outFile);
    fs.mkdirSync(path.dirname(out), { recursive: true });
