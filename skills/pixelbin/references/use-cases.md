...
End-to-end workflows you can run from a single agent prompt with this skill loaded.
...
2. The agent reads `brief.json` and produces `page-spec.json` containing:
...
**What happens:** Same as #5 but with `--brand-files "./src/styles/*.css ./src/app/page.tsx"` (or any glob). The script reads every file, extracts hex colors / fonts / max-widths / CSS variables, and includes them in the brief so the agent can match the design.
...
## Tips for prompting the agent well
